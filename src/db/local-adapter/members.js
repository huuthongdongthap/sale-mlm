...
class MembersOps {
  ...
  async getMember(id) {
    return bindFirst(this.db.prepare('SELECT id, name, email, email_encrypted, phone_encrypted, password_hash, role, tier, psn_id, status, created_at FROM members WHERE id = ?').bind(id));
  }

  async listMembers(filters = {}) {
    let query = 'SELECT id, name, email, email_encrypted, phone_encrypted, password_hash, role, tier, psn_id, status, created_at FROM members WHERE 1=1';
    const params = [];
  ...
  async createMember(data) {
    ...
    bindRun(this.db.prepare('INSERT INTO members (id, name, email, email_encrypted, phone_encrypted, password_hash, role, tier, psn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(
      id, data.name, data.email, data.email_encrypted || '', data.phone_encrypted || '', data.password_hash || '', data.role || 'Member', data.tier || 1, data.psn_id || null
    ));
