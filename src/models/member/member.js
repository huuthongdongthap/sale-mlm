...
class Member {
  constructor(data = {}) {
    this.id = data.id || require('./constants').uid();
    this.name = data.name || '';
  ...
  /**
   * Seed the database with the default member set when it is empty.
   * Mirrors the in-memory `createSeededMembers` so the DB-backed and
   * in-memory stores stay consistent.
   */
  static async seedIfEmpty(db) {
    if (!db || !db.getMember || !db.createMember) return;
    const existing = await db.listMembers ? await db.listMembers({}) : [];
    if (existing.length > 0) return;
    for (const m of Member.createSeededMembers()) {
      await db.createMember(m.toJSON ? m.toJSON() : m);
    }
  }
